...
for line in lines:
    ...
    if read_id not in read_dict:
        read_dict[read_id] = [str(tax_id)]
    else: 
        read_dict[read_id].append(str(tax_id))
...
for read in read_dict.keys():
    ...
    try:    
        lca = get_common_ancestor(ncbi_taxonomy,read_dict[read])
        if lca not in assignments:
            assignments[lca] = 1
        else: 
            assignments[lca] += 1
    except:
        not_in_accs_file.write(str(read_dict[read]))
