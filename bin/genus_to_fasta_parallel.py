...
Will need pigz, tar, GNU parallel, and blastcmd installed locally 
'''
...
# list all files in sys.argv[2] with the pattern in sys.argv[3]
file_list = glob.glob(os.path.join(sys.argv[2], sys.argv[3]))
file_list = [file for file in file_list if not file.endswith('.tar.gz') and not file.endswith('.tar.gz.md5')]
print('found', len(file_list), 'files matching the pattern')

file_list = set([os.path.splitext(file)[0] for file in file_list])
...
def process_file(file):
    ...
    fulltempfastafilename =  str(filebasename) + '.gfp.temp.fasta'
    tar_extract_cmd = 'tar -xvzf ' + sys.argv[2] + '/' + file
    fasta_extract_cmd = 'blastdbcmd -entry all -db ' + file + ' -out ' + fulltempfastafilename
    ...
    seen = {}
    with open(fulltempfastafilename) as fastafile:
        ...
        for g in seen.keys(): 
            tempfastafilename = 'genus_organized/' + str(g) + '.' + str(filebasename) + '.temp.fasta'
            record_list = seen[g]
            #print(tempfastafilename)
        ...
        compress_cmd = 'find genus_organized/ -name "*.fasta" -print0 | xargs -0  pigz -p 16'
        #subprocess.call(compress_cmd, shell = True)
    # delete and close intermediates
    fasta_remove_cmd = 'rm ' + fulltempfastafilename
